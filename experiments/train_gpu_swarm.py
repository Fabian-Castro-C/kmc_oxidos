...
def train_gpu_swarm():
    ...
    env = TensorTiO2Env(
        ...
        lattice_size=DEFAULT_CONFIG["lattice_size"],
        device=args.device
    )
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(DEFAULT_CONFIG["num_steps"]):
            ...
            coverage = (env.lattices != SpeciesType.VACANT.value).float().mean(dim=(1,2,3)).unsqueeze(1) # (B, 1)
            global_features = torch.cat([coverage, torch.zeros(B, 11, device=device)], dim=1) # (B, 12)
            ...
            env_action_indices = torch.multinomial(torch.softmax(flat_env_logits, dim=1), num_samples=1).squeeze(1)
            
            # Calculate logprob of the CHOSEN environment action
            env_probs = torch.softmax(flat_env_logits, dim=1)
            env_logprobs = torch.log(env_probs.gather(1, env_action_indices.unsqueeze(1)).squeeze(1))
        ...
        for epoch in range(DEFAULT_CONFIG["update_epochs"]):
            ...
            for start in range(0, len(inds), minibatch_size):
                ...
                mb_coverage = (mb_lattices != SpeciesType.VACANT.value).float().mean(dim=(1,2,3)).unsqueeze(1)
                mb_global = torch.cat([mb_coverage, torch.zeros(len(mb_inds), 11, device=device)], dim=1)
                ...
                pg_loss2 = -mb_adv * torch.clamp(ratio, 1 - DEFAULT_CONFIG["clip_coef"], 1 + DEFAULT_CONFIG["clip_coef"])
        ...
        logger.info(f"Update {update}/{num_updates} | FPS: {fps} | Loss: {loss.item():.4f} | Reward: {mean_reward:.4f}")
